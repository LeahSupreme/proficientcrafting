...
public class JewelcraftingStationRenderer implements BlockEntityRenderer<JewelcraftingStationBlockEntity> {
    private final ItemRenderer itemRenderer;
    private static final float PEDESTAL_TOP_HEIGHT = 0.76f;

    public JewelcraftingStationRenderer(BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public void render(JewelcraftingStationBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {
        ...
        for (int slotIndex = 0; slotIndex < blockEntity.getInputStacks().size(); slotIndex++) {
            ...
            float itemHeight = PEDESTAL_TOP_HEIGHT + (slotIndex * 0.001f);
        ...
        if (!outputStack.isEmpty()) {
            poseStack.pushPose();
            poseStack.translate(0.5, PEDESTAL_TOP_HEIGHT, 0.5);
            poseStack.mulPose(Axis.YP.rotationDegrees(facingYaw));
