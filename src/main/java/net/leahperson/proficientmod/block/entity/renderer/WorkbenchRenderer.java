...
import net.leahperson.proficientmod.block.entity.WorkbenchBlockEntity;
...
public class WorkbenchRenderer implements BlockEntityRenderer<WorkbenchBlockEntity> {
    ...
    private static final float SURFACE_HEIGHT = 1.02f;
    ...
    @Override
    public void render(WorkbenchBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {
        ...
        for (int slotIndex = 0; slotIndex < blockEntity.getInputStacks().size(); slotIndex++) {
            ...
            float itemHeight = SURFACE_HEIGHT + (slotIndex * 0.001f);
        ...
        if (!outputStack.isEmpty()) {
            poseStack.pushPose();
            poseStack.translate(0.5, SURFACE_HEIGHT, 0.5);
            poseStack.mulPose(Axis.YP.rotationDegrees(facingYaw));
