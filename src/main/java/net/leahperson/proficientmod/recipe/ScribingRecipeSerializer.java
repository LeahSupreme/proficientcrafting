...
public class ScribingRecipeSerializer implements RecipeSerializer<ScribingRecipe> {
    @Override
    public ScribingRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
        ...
        if (json.has("enchantment")) {
            enchantmentId = new ResourceLocation(GsonHelper.getAsString(json, "enchantment"));
        }
        NonNullList<Integer> enchantLevels = NonNullList.withSize(0, 0);
        if (json.has("enchantLevels")) {
            JsonArray enchLvlJson = GsonHelper.getAsJsonArray(json, "enchantLevels");
            enchantLevels = NonNullList.withSize(enchLvlJson.size(), 0);
            for (int i = 0; i < enchLvlJson.size(); i++) {
                enchantLevels.set(i, enchLvlJson.get(i).getAsInt());
            }
        }
        return new ScribingRecipe(recipeId, inputItems, output, outputs, proficiencyRequired,
                qualityRequired, qualityPerIngredient, levelCost, yieldCost, yieldAdded, craftTime,
                qualityDecoration, enchantmentId, enchantLevels);
    }

    @Override
    public @Nullable ScribingRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buf) {
        ...
        boolean hasEnchantment = buf.readBoolean();
        ResourceLocation enchantmentId = hasEnchantment ? buf.readResourceLocation() : null;
        int enchantLevelsSize = buf.readVarInt();
        NonNullList<Integer> enchantLevels = NonNullList.withSize(enchantLevelsSize, 0);
        for (int i = 0; i < enchantLevelsSize; i++) {
            enchantLevels.set(i, buf.readVarInt());
        }
        return new ScribingRecipe(recipeId, inputItems, output, outputs, proficiencyRequired,
                qualityRequired, qualityPerIngredient, levelCost, yieldCost, yieldAdded, craftTime,
                qualityDecoration, enchantmentId, enchantLevels);
    ...
    public void toNetwork(FriendlyByteBuf buf, ScribingRecipe recipe) {
        ...
        buf.writeBoolean(recipe.getEnchantmentId() != null);
        if (recipe.getEnchantmentId() != null) {
            buf.writeResourceLocation(recipe.getEnchantmentId());
        }
        buf.writeVarInt(recipe.getEnchantLevels().size());
        for (int level : recipe.getEnchantLevels()) {
